...
export default function About({innerRef}) {
    ...
    function aboutMeText() {
       ...
       /*
            ...
            [aboutjanik ~]$ Hi I'm Janik. I'm a Software apprentice at BBW-Suedhessen. I like dogs and also gaming. I'm also highly motivatat for most things.
       */
        return <>
            <p><span style={{color: info.baseColor}}>[{firstName}{info.lastName.toLowerCase()} <span className={Style.turquoise}>~</span>]$</span> cat
                about{firstName} </p>
            <p><span style={{color: info.baseColor}}>[{firstName}{info.lastName.toLowerCase()} <span className={Style.turquoise}>~</span>]$ </span>
                {info.bio}
            </p>
        </>;
    }
